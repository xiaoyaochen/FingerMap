...
class WebPage:
    ...
    def __init__(self, url:str, html:str, rurl:str, status:int,headers:Mapping[str, Any]):
        """
    ...
    async def new_from_url_async(cls, url: str, verify: bool = True,
        ...
        async with aiohttp_client_session.get(url, **kwargs) as response:
            return await cls.new_from_response_async(response,url)

    @classmethod
    async def new_from_response_async(cls, response:aiohttp.ClientResponse, rurl:str) -> 'WebPage':
        """
        Constructs a new WebPage object for the response,
        using the `BeautifulSoup` module to parse the HTML.
        ...
        return cls(str(response.url), html=html,rurl=rurl,status=status,headers=response.headers)
...
class Wappalyzer:
    ...
    def _has_technology(self, technology: Dict[str, Any], webpage: WebPage) -> bool:
        ...
        if webpage:
            ...
            webpage.headers = CIMultiDict(webpage.headers)
    ...
    def analyze(self, webpage:WebPage) -> Set[str]:
        ...
        return webpage.rurl,self.get_title(webpage.html),webpage.status,detected_technologies
    ...
    def analyze_with_versions(self, webpage:WebPage) -> Dict[str, Dict[str, Any]]:
        ...
        for app_name in detected_apps[3]:
            versions = self.get_versions(app_name)
            versioned_apps[app_name] = {"versions": versions}
        return webpage.rurl,self.get_title(webpage.html),webpage.status,versioned_apps
